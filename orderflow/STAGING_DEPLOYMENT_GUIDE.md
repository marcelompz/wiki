...
### **Paso 6: Configurar Nginx para Staging**

```bash
# Instalar Nginx (si no está)
apt-get update && apt-get install -y nginx

# Configurar reverse proxy para staging
cat > /etc/nginx/sites-available/staging.provecchio.com << 'EOF'
server {
    listen 80;
    server_name staging.provecchio.com;
...
# Habilitar sitio
ln -s /etc/nginx/sites-available/staging.provecchio.com /etc/nginx/sites-enabled/
