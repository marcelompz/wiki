...
* Múltiples fotografías por producto y por variante (con fallback automático a las del template).  
...
* Modelo de precios desacoplado: Precio Base en Template \+ Sobreprecio (`priceDelta`) en Variante.  
...
* **Regla arquitectónica inmutable: Precio base en Product (template), sobreprecio y stock siempre en ProductVariant** (incluso si es la única variante default — simplifica y unifica el resto del sistema).
...
* Módulo `VariantsModule`:  
  ...
  * Endpoint de actualización masiva (Bulk Update): actualización de sobreprecios (`priceDelta`) o stock filtrada por valor de atributo (ej. "+$500 a todos los Talle=G").  
  * CRUD puntual de variante: ajuste fino de SKU, código de barras, stock, delta y fotos específicas.  
...
* Diseño de la plantilla estándar unificada: estructura tabular donde productos sin variantes ocupan 1 fila y productos con variantes ocupan N filas consecutivas agrupadas por la columna `handle`.  
...
* Vista tipo **matriz/spreadsheet** para edición bulk de sobreprecio/stock por variante (evitar formulario uno por uno), con la posibilidad de aplicar un ajuste a toda una fila o columna de atributo de una sola vez (ej. "+$500 a todos los G").  
...
* Cada variante tiene su propio SKU único, código de barras, stock, costo y un sobreprecio (priceDelta) que se suma al precio base del template — el precio base vive únicamente en el template y nunca se duplica por variante, para que un cambio de precio base se propague automáticamente a todas las variantes. Cada variante puede además tener fotografías propias; si una variante no tiene fotos propias, se muestran las del template como fallback.  
...
1\. Schema completo de Prisma para: Product (template), Attribute, AttributeValue, ProductAttributeLine (con relación many-to-many a AttributeValue para el subconjunto seleccionado), ProductVariant, VariantAttributeValue, Photo, ImportBatch — con relaciones, índices y constraints de unicidad necesarios.  
