...
```
Request → AuthGuard (extrae tenantId)
       → TenantContext / TenantResolver
       → ¿Tier del tenant?
            ├─ Shared  → Prisma client global + filtro tenantId
            └─ Dedicated → Prisma client específico del tenant (connection string propia)
       → Servicio de BI (@TenantPrisma)
       → $queryRaw / findMany / agregaciones
```
