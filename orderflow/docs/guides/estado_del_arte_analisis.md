# Análisis del Estado del Arte — OrderFlow v1.1.9 (Actualización)

**Fecha:** 2026-07-31 · **Versión actual:** `v1.1.9` · **Siguiente target:** `v2.0.0`
...
OrderFlow alcanzó su **v1.0.0 Commercial Release** el 25 de julio de 2026 y está ahora en **v1.1.9** con microservicios standalone, soft-delete de tenants y WebSockets escalables con Redis. En ~16 días (15 Jun → 31 Jul) pasó de un MVP básico (v0.1.0) a una **plataforma SaaS comercial completa** con 38 modelos Prisma, billing, marketplace, 6 microservicios standalone y estructura Kubernetes preparada.
...
timeline
    ...
    2026-07-18 : v0.5.0 Traefik v3.4 + App Store
...
| **RBAC granular** | ⚠️ Solo ProductsController | ✅ 21/33 controllers protegidos | **~64% CERRADO** |
...
| **Testing 80%** | ~30-35% | ~45% (389 tests / 50 suites) | **PARCIAL** |
...
## 3. Matriz de Madurez — Recalculada v1.1.9

| Área | Eval v0.5.0 | Eval v1.1.9 | Δ | Justificación |
|------|-------------|-------------|---|---------------|
...
| **Testing** | 6.5/10 | **7.0/10** | +0.5 | 389 tests / 50 suites (vs 305/41). Mejora, pero cobertura real sigue ~40-45%. Frontend unit tests siguen casi inexistentes (2 archivos). |
...
- RBAC granular en **21 de 33 controllers** (64%):
  - ✅ Con RBAC: billing, backups, biolinks, bookings, contacts, customers, giveaways, integrations, loyalty, marketplace, notifications, orders, product-imports, products, quotations, system-modules, users, analytics, whatsapp-catalog-admin, whatsapp-catalog-superadmin
  - ❌ Sin RBAC: auth, health, root-health, metrics, tenants (parcialmente intencional — auth/tenants/métricas/salud tienen su propio auth flow)
...
- 6 releases en los últimos 8 días (v0.5.1 → v1.1.0) y 9 releases más hasta **v1.1.9** (31 Jul)
...
|------|--------|---------------|--------|
| Unit tests backend | 305 tests / 41 suites | **389 tests / 50 suites** | 80%+ cobertura |
...
> [!WARNING]
> El testing sigue siendo el **gap más importante**. Aunque hubo mejora (305→389 tests), la cobertura real probablemente no supera el 45%. Los frontend unit tests son prácticamente inexistentes (2 archivos). Para un SaaS comercial v1.0, esto es un riesgo.
...
| `root-health` | ❌ | Intencional (health check raíz) |
| `metrics` | ❌ | Debería tener `metrics:read` (solo SuperAdmin) |
| `tenants` | ⚠️ Parcial | Intencional (tiene `assertCanManageTenant` propio, no PermissionsGuard) |
| `public-orders` | ❌ | Intencional (endpoint público de checkout) |
| `public-catalog` | ❌ | Intencional (catálogo público) |
| `public-products` | ❌ | Intencional (productos públicos) |
| `public-storefront` | ❌ | Intencional (storefront público) |
| `sync-products` | ❌ | Intencional (endpoint de sync, protegido por API key) |
| `orderflow-integration` | ❌ | Intencional (webhook interno Odoo) |
| `whatsapp-catalog-webhooks` | ❌ | Intencional (webhook público) |

> De estos 12, **4 son intencionales** (auth, health, root-health, public/* endpoints) y **2 deberían protegerse** (metrics, tenants parcial). El resto son webhooks/sync públicos protegidos por API key.
...
| **Métricas desactualizadas en ROADMAP** (L319): "305 passing / 41 suites" | 🔴 **Nuevo** | Actual: 389 tests / 50 suites |
...
```mermaid
graph TD
    A["🔴 P0: Testing 80%<br/>Unit + E2E + Frontend tests<br/>389→600+ tests"] --> B["🔴 P0: Cleanup Documentación<br/>ROADMAP deudas, CHANGELOG orden,<br/>featurelist.json sync"]
...
OrderFlow completó un **salto cualitativo excepcional** en 8 días — de una plataforma sólida pero sin billing (v0.5.0, madurez 7.7/10) a un **SaaS comercial completo** (v1.1.9, madureza ~9.0/10). Los 3 gaps más críticos del análisis anterior (billing, marketplace, microservicios) están **completamente cerrados**.
...
| Dimensión | v0.5.0 (Jul 19) | v1.1.9 (Jul 31) |
|-----------|-----------------|------------------|
