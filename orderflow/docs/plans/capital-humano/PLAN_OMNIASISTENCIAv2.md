...
- **Vinculación Tripartita (Contact ↔ Employee ↔ User):** Se sigue la convención Odoo (`res.partner` ↔ `hr.employee` ↔ `res.users`). `Contact` es la entidad raíz de identidad física (`taxId`/cédula), `Employee` contiene el legajo laboral y marcaciones (`contactId`), y `User` es la cuenta opcional de acceso al sistema (`userId`). Un colaborador puede existir como `Employee` para marcación sin requerir una cuenta de `User`.
...
model ShiftTemplate {
  ...
  tenantId           String
  name               String   // ej: "Turno Mañana 08:00 - 17:00", "Turno Nocturno"
  startTime          String   // HH:mm (08:00)
  endTime            String   // HH:mm (17:00)
  breakDurationMins  Int      @default(60)
  gracePeriodMins    Int      @default(10) // tolerancia antes de marcar tardanza
  isOvernight        Boolean  @default(false) // cruza medianoche
  createdAt          DateTime @default(now())

  workSchedules      WorkScheduleDetail[]
...
model WorkSchedule {
  id            String       @id @default(cuid())
  tenantId      String
  name          String       // ej: "Jornada Administrativa 40h", "Rotativo Operativo"
...
model Holiday {
  id           String   @id @default(cuid())
  tenantId     String
  name         String   // ej: "Día de la Independencia"
  date         DateTime
  jurisdiction String   @default("PY")
  isMandatory  Boolean  @default(true) // feriado inamovible / pago doble
