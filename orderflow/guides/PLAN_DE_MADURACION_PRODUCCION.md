...
- **Frontend (`Dockerfile.prod`):** Creado usando Alpine, sirve build estático con `serve` y configuración SPA estricta (`frontend/serve.json`). El enrutamiento y SSL lo maneja Traefik v3.4.
- **NGINX Edge (legacy):** La configuración edge-proxy legacy de NGINX fue reemplazada por Traefik v3.3. Los archivos históricos se preservan en `old/` hasta la próxima fase de madurez.
...
- Implementados health checks automáticos en el script de despliegue para validar Traefik, Backend y Frontend.
